...
class Node:
    ...
    def get_features_to_check_random(self,num_columns):
        num_columns = num_columns - 1
        squre_features = int(round(np.sqrt(num_columns)))
        the_range = np.random.randint(num_columns, size=squre_features)
        return the_range
    ...
    def get_best_split_random(self):
        num_rows = self.dataset.shape[0]
        num_columns = self.dataset.shape[1]
        best_score = 999
        features_to_search = self.get_features_to_check_random(num_columns)
    ...
    def split_node_recursively_random(self,max_depth = 6):
        purity = self.calc_node_purity()
        if self.depth_in_tree > max_depth or purity > self.purity_to_stop_split:
            self.label = self.calc_majority_label()
            return
        else:
            self.feature_index,self.value,right_dataset,left_dataset = self.get_best_split_random()
            self.right = Node(right_dataset,self.depth_in_tree+1,self.is_gini)
            self.right.split_node_recursively_random(max_depth)
            self.left = Node(left_dataset,self.depth_in_tree+1,self.is_gini)
            self.left.split_node_recursively_random(max_depth)
...
def subsample_create(data, ratio):
    rows_count = data.shape[0]
    sample_size = int(round(rows_count * ratio))
    idx = np.random.randint(rows_count, size=sample_size)
    return data[idx,:]
...
def random_forest(train, test, max_depth, sample_size, n_trees):
    ...
    for i in range(n_trees):
        sample = subsample_create(train, sample_size)
        tree = Node(sample, 1)
        tree.split_node_recursively_random(max_depth)
        trees.append(tree)
...
if __name__ == '__main__':
    ...
    #2 
    test_y = random_forest(data,data_test,6, 0.8, 10)
    accuracy = 100*(1-np.sum(abs(y_test - test_y)/len(y_test)))
    print('accuracy is {}'.format(accuracy))
